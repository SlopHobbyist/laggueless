...
3. ~~**WASAPI exclusive-mode opt-in.** Shared mode floors at the engine period (~10 ms). Exclusive mode can reach ~3 ms. Add an opt-in toggle in [settings.yaml](settings.yaml) for users with dedicated audio hardware.~~ ✅ `audio.exclusive_mode: true` in settings.yaml (or `--exclusive-audio` CLI flag) attempts AUDCLNT_SHAREMODE_EXCLUSIVE with a 16-bit PCM format at the device's native rate, targeting ~3 ms period (clamped to device minimum). Handles the AUDCLNT_E_BUFFER_SIZE_NOT_ALIGNED retry. Falls back to shared mode with a warning if the device rejects exclusive access.
...
7. ~~**Per-stage latency telemetry.** Existing `--timing-log` and `--pace-log` are coarse. Add input→poll→core→present→photons breakdown with frame indices so regressions are visible at a glance.~~ ✅ New `--latency-log` flag (also `log.latency_log` in settings.yaml) prints per-second averages of: DXGI backpressure wait, pre-poll core time, post-poll core time, present, and pacing-wait — plus the worst iteration time, missed-poll count, and the frame-index range the window covers.
