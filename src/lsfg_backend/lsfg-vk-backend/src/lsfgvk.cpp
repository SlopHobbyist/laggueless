...
namespace {
    ...
    std::filesystem::path findCacheFilePath() {
        ...
        if (appdata && *appdata != '\0')
            return std::filesystem::path(appdata) / "lsfg-vk" / "lsfg-vk_pipeline_cache.bin";
        return{"lsfg-vk_pipeline_cache.bin"};
    }
...
Context& Instance::openContext(std::pair<HANDLE, HANDLE> sourceHandles, const std::vector<HANDLE>& destHandles,
        HANDLE syncHandle, uint32_t width, uint32_t height,
...
namespace {
    /// import source images (Win32 HANDLE)
    std::pair<vk::Image, vk::Image> importImages(const vk::Vulkan& vk,
            const std::pair<HANDLE, HANDLE>& sourceHandles,
            VkExtent2D extent, VkFormat format) {
        try {
            return {
                vk::Image(vk, extent, format,
                    VK_IMAGE_USAGE_STORAGE_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, sourceHandles.first),
                vk::Image(vk, extent, format,
                    VK_IMAGE_USAGE_STORAGE_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, sourceHandles.second)
            };
        } catch (const std::exception& e) {
            throw backend::error("Unable to import source images", e);
        }
    }
    /// import destination images (Win32 HANDLE)
    std::vector<vk::Image> importImages(const vk::Vulkan& vk,
            const std::vector<HANDLE>& destHandles,
            VkExtent2D extent, VkFormat format) {
        try {
            std::vector<vk::Image> destImages;
            destImages.reserve(destHandles.size());

            for (const auto& h : destHandles)
                destImages.emplace_back(vk, extent, format,
                    VK_IMAGE_USAGE_STORAGE_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, h);
    ...
    /// import timeline semaphore (Win32 HANDLE)
    vk::TimelineSemaphore importTimelineSemaphore(const vk::Vulkan& vk, HANDLE syncHandle) {
        try {
